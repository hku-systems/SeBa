...
static long sgx_ocall_setsockopt(void* pms) {
    ...
    if (ms->ms_optname == 20 || ms->ms_optname == 21) {
        struct timeval tv;
        tv.tv_sec = 5;
        tv.tv_usec = 0;
        ret = INLINE_SYSCALL(setsockopt, 5, ms->ms_sockfd, ms->ms_level, ms->ms_optname, (const void*)&tv,
                sizeof(tv));
    } else {
...
static long sgx_ocall_ioctl(void* pms) {
    ...
    struct ifconf conf;
    if (ms->ms_type == SIOCGIFCONF) {
        conf.ifc_ifcu.ifcu_buf = ms->ms_ifc;
        conf.ifc_len = ms->ms_ifc_len;
        ret = INLINE_SYSCALL(ioctl, 3, sockfd, ms->ms_type, &conf);
        ms->ms_ifc_len = conf.ifc_len;
    } else {
        ret = INLINE_SYSCALL(ioctl, 3, sockfd, ms->ms_type, ms->ms_ifc);
    }
    close(sockfd);
    return ret;
}
